...
class Rpt extends App_Controller{
	...
	public function submit_crf_rpt() {
		...
		$tax_year_paid = $this->input->post('tax_year_paid', TRUE) ?: null;

		//log_message('debug', "CRF FORM SUBMIT: pr_id=$pr_id, is_no=$is_no, type=$type");

		// Conditional validation: Only required if type is RPT
		if ($type === 'RPT' && empty($tax_year_paid)) {
			$this->session->set_flashdata('error', 'Tax Year to be Paid is required for Real Property Tax (RPT).');
			redirect('Rpt/Rpt_table/');
			return;
		}
		...
		// Only add tax_year_paid if type is RPT
		if ($type === 'RPT') {
			$data['tax_year_paid'] = $tax_year_paid;
		}
