							...
							<ul class="nav side-menu">
								...
								<?php if ($this->session->userdata('user_type') == "Administrator"){ ?>
									...
									<li><a><i class="fa fa-users"></i> User Accounts<span class="fa fa-chevron-down"></span></a>
										<ul class="nav child_menu">
											<li><a href="<?= base_url('Admin/create_account'); ?>"><i class="fa fa-plus-square"></i> Create Account</a></li>
								...
								<?php if ($this->session->userdata('user_type') == "CCD"){ ?>
									...
									<li><a><i class="fa fa-edit"></i> Aspayment <span class="fa fa-chevron-down"></span></a>
										<ul class="nav child_menu">
											<li><a href="<?= base_url('Aspayment/judicial');?>">Judicial</a></li>
				...
				<div class="nav_menu">
					<nav>
						...
						<ul class="nav navbar-nav navbar-right" id="load_tap_nav">
							<li><a data-toggle="tooltip" title="logout" class="user-log-out" data-placement="top" href="<?php echo site_url(); ?>logout">
							...
							<?php if ($this->session->userdata('user_type') == "Secretary" || $this->session->userdata('user_type') == "CCD"){ ?>
								<li role="presentation" class="dropdown notif-tab" onclick="scrollSmoothToBottom()">
									<a href="javascript:;" class="dropdown-toggle info-number" data-toggle="dropdown" aria-expanded="false">
										<i class="fa fa-envelope-o"></i><span class="badge bg-green" id="no_of_notif_tab">0</span>
									</a>
									<ul role="menu" class="dropdown-menu list-unstyled msg_list" id="xz">
										...
										?>
											...
											>
												<a 
													<?php if($value['form_type'] == 'IS' && $value['action'] == 'disapproved'){ ?>
														href = "<?php echo base_url('requests/view_interview_sheet/'.$value['reference_id']) ?>"
													<?php }elseif($value['form_type'] == 'IS' && $value['action'] == 'approved'){ ?>
														href = "<?php echo base_url('Secretary/Progress/view_inprogress/'.$value['reference_id']) ?>" 
													...
													<?php }elseif($value['form_type'] == 'LCAF' && $value['action'] == 'disapproved'){ ?>
														href = "<?php echo base_url('requests/disapproved_payments') ?>" 
												...
												>
													...
													<?php } ?>
													<span>
														<span><?php echo $value['form_type']; ?> No. <?php echo $value['reference_id']; ?></span>
														<span class="time"></span>
													</span>
													<span class="message">has been <?php echo $value['action']; ?> on <?php echo $action_date; ?></span>
												</a>
											</li>
										<?php } if($all_notification_no == 0){ ?>
											<li><div class="text-center"><strong>No notification to be shown</strong></div></li>
										<?php }?>
										<li><a href="<?php echo base_url('notification') ?>" style="margin-left: 50px;"><strong>See All Notifications </strong><i class="fa fa-angle-right"></i></a></li>
									</ul>
								</li>
							<?php } ?>
						</ul>
