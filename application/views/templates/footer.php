...
<script>
	$(document).ready(function(){
		...
		let rpt_tables = {}; // define it globally

		function initializeRPTTable(tableId, status) {
			// rpt_table = $('#' + tableId).DataTable({//RPT TABLE
			rpt_tables[tableId] = $('#' + tableId).DataTable({
			    fixedHeader: false,
			    processing: true,
			    serverSide: true,
			    order: [],
		...
		initializeRPTTable('rpt_datatable');
		initializeRPTTable('pending_rpt_datatable', 'Pending');
		initializeRPTTable('paid_rpt_datatable', 'Paid');
		...
		$('#searchBtn').on('click', function () {
		    let visibleTableId = $('.dataTable:visible').attr('id');
		    if (rpt_tables[visibleTableId]) {
		        rpt_tables[visibleTableId].draw();
		    }
		});

		$('#landprofile_datatable').DataTable({//LAND PROFILE TABLE
			fixedHeader: false,
			"processing": true,
			"serverSide": true,
			"order": [],
			"ajax": {
				"url": "<?php echo base_url('Land_Profile/LandProfile_datatable/'); ?>",
				"type": "POST"
			},
			"columnDefs": [{ 
				"targets": [0],
...
<script type="text/javascript">//Land Profile
	$('#lot_info').submit(function(event) {
		var id 	= $(".v-identifier").attr('id');
		var req = $.ajax({
			type: "POST",
			url: "<?php echo base_url('Land_Profile/view_document/"+id+"'); ?>",
